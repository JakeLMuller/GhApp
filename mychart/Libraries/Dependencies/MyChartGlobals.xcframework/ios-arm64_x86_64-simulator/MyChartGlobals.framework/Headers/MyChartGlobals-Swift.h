...
/// Access result values for accessing MyChart standard activities
...
/// Status values for the Appointment Arival Feature (“Hello Patient”)
typedef SWIFT_ENUM(NSInteger, WPAPIAppointmentArrivalFeatureStatus, open) {
/// Hello Patient is turned off
  WPAPIAppointmentArrivalFeatureStatusDisabled = 1,
/// The Hello Patient onboarding hasn’t been shown
  WPAPIAppointmentArrivalFeatureStatusOnboardingNotShown = 2,
/// Hello Patient is missing the required setup (e.g. location permissions)
  WPAPIAppointmentArrivalFeatureStatusMissingSetupRequirement = 3,
/// Hello Patient is turned on
  WPAPIAppointmentArrivalFeatureStatusEnabled = 4,
};

/// The self-arrival mechanism by which the user arrived to an appointment
...
SWIFT_PROTOCOL("_TtP14MyChartGlobals29WPAPICreateMyChartURLDelegate_")
...
///  Declares a delegate protocol for callers to implement to be notified of Push Notifications status changes
SWIFT_PROTOCOL("_TtP14MyChartGlobals30WPAPIPushNotificationsDelegate_")
@protocol WPAPIPushNotificationsDelegate
...
///
- (void)setPushNotificationsReturnedWithStatus:(enum WPAPISetPushNotificationsStatusResult)status;
...
///
- (void)pushNotificationsReturnedWithStatus:(enum WPAPIPushNotificationsStatus)status;
@end

/// A result status for Push Notification availability
...
/// A result status for when toggling Push Notifications is attempted
